...
export function useStaffImport() {
  ...
  const [preview, setPreview] = useState<ParsedStaff[] | null>(null);
  ...
  const parse = async (data: Uint8Array, ext: string): Promise<ParsedStaff[]> => {
    ...
    if (ext === 'csv') {
      ...
      raw = XLSX.utils.sheet_to_json(ws, { raw: false, defval: '' }) as any[];
    }

    const get = (r: any, keys: string[], allKeys: string[]) => {
      for (const n of keys) {
        const k = allKeys.find(k => k.toLowerCase().replace(/[\s._-]/g, '') === n.toLowerCase().replace(/[\s._-]/g, ''));
        if (k !== undefined) return (r[k] ?? '').toString().trim();
      }
      return '';
    };
    ...
    return raw.map(r => {
      ...
      if (!id && !first && !last) return null as any;
      return { id, first, last, dept, market, phone, role, salary, authLevel } as ParsedStaff;
    }).filter(Boolean);
  };
